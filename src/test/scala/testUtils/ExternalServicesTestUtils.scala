...
import uk.nationalarchives.dri.Lambda
...
class ExternalServicesTestUtils extends AnyFlatSpec with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  case class MockLambda(
    ...
    val publishEntitiesCaptor: ArgumentCaptor[List[Entity]] = ArgumentCaptor.forClass(classOf[List[Entity]])
    ...
    override def dASnsDBClient: DASNSClient[IO] = {
      when(mockSnsClient.publish(any[String])(any[List[Entity]])(any[Encoder[Entity]]))
    ...
    def verifyInvocationsAndArgumentsPassed(
      ...
      )(publishEntitiesCaptor.capture())(any[Encoder[Entity]])
